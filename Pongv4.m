...
bufferSize = 1; %Number of readings to average for smoothing
accelBuffer = zeros(bufferSize,1);
bufferIndex = 1;

moveThresh = 5; %Sets the acceleration threshold (To change)
paddleMove = 10; %How much the player block needs to move
lastAccelValue = 0; %Used to store the last accelerometer reading
...
while toc < 600 %Sets a timer for 10 minutes
    ...
    accelBuffer(bufferIndex) = y_accel;
    bufferIndex = mod(bufferIndex, bufferSize) + 1; %Loops the index
    smoothedAccel = mean(accelBuffer); %Calculates the average

    if abs(smoothedAccel - lastAccelValue) > moveThresh %If the smoothed acceleration > the most recent acceleration
        if smoothedAccel < lastAccelValue %If smoothed > most recent
            blockA = blockA + paddleMove; %Move to the right by 10
        else
            blockA = blockA - paddleMove; %Move to the left by 10
        end

        blockA = max(min(blockA, 100 - paddleSize),paddleSize); %Makes sure block A stays within game bounds
        lastAccelValue = smoothedAccel; %Update the most recent acceleration
    end
